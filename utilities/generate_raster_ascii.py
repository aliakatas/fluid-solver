...
filenameGradWithFeature = 'dummy_raster_grad_feature.asc'
...
############################################
def generate_grid_with_gradient(cols, rows, xll, yll, cell, nodata, low=0.1, high=10.):
    '''
    '''
    r = high - low
    ...
    for irow in range(rows):
        for icol in range(cols):
            temp[irow, icol] = low + irow * r + icol * r
    return temp

############################################
def embed_feature(cols, rows, data, nodataval):
    '''
    '''
    midcol = int(cols / 2)
...
if __name__ == '__main__':
    ...
    grad_data = generate_grid_with_gradient(ncols, nrows, xllCorner, yllCorner, cellSize, NODATA_d)
    grad_data = embed_feature(ncols, nrows, grad_data, NODATA_d)
    write_to_ascii(filenameGradWithFeature, ncols, nrows, xllCorner, yllCorner, cellSize, NODATA_d, grad_data)
